...
This module provides common functions for working with Microsoft Fabric,
including data processing, metadata management, and workspace operations.
"""

# Fabric imports with better error handling
try:
    import sempy.fabric as fabric
except ImportError:
    fabric = None
    print("Warning: semantic-link package not available. Install with: %pip install semantic-link")
...
# notebookutils is a global variable in Fabric notebooks, not an importable module
notebookutils = None
mssparkutils = None
...
__all__ = [
    ...
    # Note: notebookutils and mssparkutils are intentionally excluded
    # to prevent overriding the notebook's global variables
]

# Function to initialize Fabric context from the calling notebook
def _initialize_fabric_globals(notebook_globals=None):
    """
    Initialize fabric globals from the notebook context.
    This should be called from initialize_fabric_context().
    """
    global notebookutils, mssparkutils
    
    if notebook_globals is None:
        # Try to get the caller's globals (this is tricky but sometimes works)
        import inspect
        frame = inspect.currentframe()
        try:
            # Go up the call stack to find the notebook frame
            caller_frame = frame.f_back.f_back  # Skip this function and initialize_fabric_context
            if caller_frame:
                notebook_globals = caller_frame.f_globals
        finally:
            del frame
    
    if notebook_globals:
        notebookutils = notebook_globals.get('notebookutils', None)
        mssparkutils = notebook_globals.get('mssparkutils', None)
        
        # Also try from builtins in case they're there
        if notebookutils is None:
            import builtins
            notebookutils = getattr(builtins, 'notebookutils', None)
        if mssparkutils is None:
            import builtins
            mssparkutils = getattr(builtins, 'mssparkutils', None)
    
try:
    # Import Spark modules but don't create session until needed
    from pyspark.sql import SparkSession
    from pyspark import SparkContext
    spark = None  # Will be initialized lazily
    sc = None     # Will be initialized lazily
except ImportError:
    SparkSession = None
    SparkContext = None
    spark = None
...
def _get_spark_session():
    ...
    global spark, sc
    if spark is None and SparkSession is not None:
        spark = SparkSession.getActiveSession()
        if spark is None:
            # Only create if we're in a Fabric environment
            try:
                spark = SparkSession.builder.getOrCreate()
                sc = spark.sparkContext
            except Exception:
                # If Spark fails to initialize, leave it as None
                pass
    return spark

# Import datetime module properly
import datetime

# Helper function to safely get Kusto token
def get_kusto_token(notebook_utils=None):
    """Safely get Kusto access token with proper error handling.
    
    Args:
        notebook_utils: Optional notebookutils object from calling notebook.
                       If not provided, uses the global notebookutils.
    """
    utils = notebook_utils or notebookutils
    if utils is None:
        raise ImportError(
            "notebookutils not available. This function requires a Microsoft Fabric environment. "
            "Try calling initialize_fabric_context() first or pass notebookutils directly."
        )
    try:
        return utils.credentials.getToken('kusto')
    except Exception as e:
        raise RuntimeError(f"Failed to get Kusto access token: {e}")
...
def GetVariableLibraryName() -> str:
    ...
    cached_name = spark_session.conf.get("fabric.variable_library_name", None)
    if cached_name:
        return cached_name
    
    try:
        # Get workspace ID for API call
        workspace_id = spark_session.conf.get("fabric.workspace_id", None)
        if not workspace_id:
            if fabric is None:
                raise RuntimeError("fabric package not available and no cached workspace ID")
            workspace_id = fabric.get_workspace_id()
            spark_session.conf.set("fabric.workspace_id", workspace_id)
        
        # Use Fabric REST API to get variable libraries
        if fabric is None:
            raise RuntimeError("fabric package not available for API calls")
        ...
        if matching_library:
            # Cache the result
            spark_session.conf.set("fabric.variable_library_name", matching_library)
        ...
        try:
            if fabric is None:
                raise RuntimeError("fabric package not available for fallback API calls")
            client = fabric.FabricRestClient()
            response = client.get(f"/v1/workspaces/{workspace_id}/items?type=VariableLibrary")
            variable_libraries = response.json()["value"]
            ...
            spark_session.conf.set("fabric.variable_library_name", fallback_library)
...
def GetVariableLibrary():
    ...
    """
    if notebookutils is None:
        raise RuntimeError("notebookutils not available. Call configure_fabric_globals() first.")
...
def GetFabricIds() -> tuple:
    ...
    def get_spark_conf(key: str) -> str:
        # Ensure Spark session is initialized before accessing config
        spark_session = _get_spark_session()
        if spark_session is None:
            return None
        return spark_session.conf.get(key, None)  # Returns None if the key does not exist
    ...
    if not workspace_id:
        if fabric is None:
            raise ImportError("semantic-link package not available. Install with: %pip install semantic-link")
        try:
            workspace_id = fabric.get_workspace_id()
        except Exception as e:
            raise RuntimeError(f"Failed to get workspace ID: {e}. Make sure you're running in a Fabric environment.")
        spark_session = _get_spark_session()
        if spark_session:
            spark_session.conf.set("fabric.workspace_id", workspace_id)


    def GetLakehouses(workspace_id=None, max_retries=10, base_delay=5) -> tuple:
        ...
        try:
            ...
            # Fallback: Retrieves all Lakehouses within the given workspace using Fabric REST API.
            # More reliable than notebookutils.lakehouse.list() which may not be available.
            
            # Args:
            #     workspace_id (str, optional): The ID of the workspace (not used in this implementation).
            ...
            while retries < max_retries:
                try:
                    # Use Fabric REST API instead of notebookutils (more reliable)
                    workspace_items = fabric.list_items()
                    lakehouses = workspace_items[workspace_items['Type'] == 'Lakehouse']
                    
                    # Convert to expected format
                    lakehouse_list = []
                    for _, lh in lakehouses.iterrows():
                        lakehouse_list.append({
                            "displayName": lh['Display Name'],
                            "id": lh['Id']
                        })
            ...
            raise Exception(f"Failed to retrieve lakehouses after {max_retries} retries.")


    # If lakehouse IDs are not stored, fetch them
    if not bronze_lakehouse_id or not silver_lakehouse_id or not gold_lakehouse_id:
        ...
        if spark_session:
            if bronze_lakehouse_id:
                spark_session.conf.set("fabric.bronze_lakehouse_id", bronze_lakehouse_id)
            if silver_lakehouse_id:
                spark_session.conf.set("fabric.silver_lakehouse_id", silver_lakehouse_id)
            if gold_lakehouse_id:
                spark_session.conf.set("fabric.gold_lakehouse_id", gold_lakehouse_id)


    return workspace_id, bronze_lakehouse_id, silver_lakehouse_id, gold_lakehouse_id

# Global Variables for Fabric IDs
# These will be initialized when first accessed
WORKSPACE_ID = None
BRONZE_LAKEHOUSE_ID = None
SILVER_LAKEHOUSE_ID = None
GOLD_LAKEHOUSE_ID = None

def configure_fabric_globals(notebook_utils=None,msspark_utils=None):
    """Explicitly configure fabric globals from the notebook context.
    
    This is the recommended way to ensure the library has access to notebookutils.
    
    Args:
        notebook_utils: The notebookutils object from your Fabric notebook
    ...
    if msspark_utils is not None:
        mssparkutils = msspark_utils

    print(f"Fabric globals configured: notebookutils={'available' if notebookutils else 'None'}")

def initialize_fabric_context():
    """Initialize global Fabric context variables.
    
    This function should be called before using functions that depend on
    workspace and lakehouse IDs. 
    
    If you get errors about notebookutils not being available, call 
    configure_fabric_globals(notebookutils, mssparkutils) first.
    """
    global WORKSPACE_ID, BRONZE_LAKEHOUSE_ID, SILVER_LAKEHOUSE_ID, GOLD_LAKEHOUSE_ID
    
    # Try to auto-detect Fabric globals from the calling notebook (may not always work)
    if notebookutils is None:
        import inspect
        frame = inspect.currentframe()
        try:
            caller_globals = frame.f_back.f_globals if frame.f_back else None
            _initialize_fabric_globals(caller_globals)
        finally:
            del frame
    
    # Initialize workspace and lakehouse IDs
...
def SetSessionLogURI(eventhouseName, databaseName):
    if (eventhouseName != "" and databaseName != ""):
        try:
            ...
            spark_session = _get_spark_session()
            workspaceId = spark_session.conf.get("trident.workspace.id") if spark_session else None
            if not workspaceId:
            ...
            spark_session = _get_spark_session()
            if spark_session is None:
                raise RuntimeError("Spark session not available. Make sure you're running in a Fabric environment.")
            
            workspaceId = spark_session.conf.get("trident.workspace.id")
            client = fabric.FabricRestClient()
            ws_eventhouses = fabric.FabricRestClient().get(f"/v1/workspaces/{workspaceId}/eventhouses").json()["value"]
        ...
        spark_session = _get_spark_session()
        if spark_session:
            spark_session.conf.set("fabric.log_eventhouse_id", eventhouse_id)
            spark_session.conf.set("fabric.log_kqldatabase_id", kqldatabase_id)
            spark_session.conf.set(f"kustoUri",kustoUri)
            spark_session.conf.set(f"kustoDatabase",databaseName)
...
def GetConfigMetadataFromYaml(model: str, objectName: str, layer: str) -> dict:
    ...
    if spark_session is None:
        raise RuntimeError("Spark session not available. Make sure you're running in a Fabric environment.")
    yaml_content = spark_session.read.text(config_file_path).collect()
...
def GetConfigMetadata(model: str, objectName: str, layer: str) -> dict:
    ...
    spark_session = _get_spark_session()
    use_yaml = spark_session.conf.get("UseYaml", "false").lower() == "true" if spark_session else False
...
def GetSourcePathLastFileFromKQL(
    ...
    try:
        ...
        result_df = (
            spark_session.read.format("com.microsoft.kusto.spark.synapse.datasource") \
            .option("kustoCluster", kustoUri) \
            .option("kustoDatabase", database) \
            .option("kustoQuery", kql_query)
            .option("accessToken", get_kusto_token())\
            .load()
        )
...
def GetSourcePathLastFilesByDatesFromKQL(
    ...
    spark_session = _get_spark_session()
    kustoUri = spark_session.conf.get(f"kustoUri") if spark_session else None
    # The database to write the data
    database = spark_session.conf.get(f"kustoDatabase") if spark_session else None
...
def UnkValues(inputDF = None):
    ...
    if use_yaml:
        ...
        unkConfigDF = spark_session.read.format("delta").load(
        ...
        initDF = spark_session.createDataFrame(sc.emptyRDD(), inputSchema)

        emptyDF = spark_session.createDataFrame([("a",)], ["aux"])
...
def UnkValuesFromYaml(inputDF=None, configPath="config/inferredMembersConfig.yml"):
    ...
    try:
        ...
        spark_session = _get_spark_session()
        yaml_content = spark_session.read.text(full_config_path).collect()
        yaml_string = "\n".join(row.value for row in yaml_content)
        config_data = yaml.safe_load(yaml_string)
...
def MergeDims(tempTable, skColumn, codColumn, tableFilePath):
    ...
    spark_session = _get_spark_session()
    persistentDimDF = spark_session.read.format("delta").load(tableFilePath)
...
def _batchProcessHistoricalChanges(stageWithOrder, persistentDF, tableDest, skName, codColumn, historicalType2Columns, scdType, historicalType3Columns, dateList):
    ...
    try:
        ...
        spark_session = _get_spark_session()
        materializedChangesDF = spark_session.read.format("delta").load(temp_path).cache()
...
def UpdateProcessingDateIntoKQL(modelName, tableName, processingDate, scope):
    ...
    accessToken = get_kusto_token()

    df = spark.createDataFrame(
        [
            (modelName, tableName, scope, processingDate)
        ],
        ["model", "tableName", "scope", "startDate"]  # add your column names here
    )
